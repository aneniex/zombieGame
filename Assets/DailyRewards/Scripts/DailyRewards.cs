    ...
    public class DailyRewards : DailyRewardsCore<DailyRewards>
    {
        ...
        public void ClaimPrize()
        {
            if (availableReward > 0)
            {
                ...
                if (rewards[availableReward-1].unit == "Gems")
                {
                    var newGems = PlayerPrefs.GetInt(PlayerPrefsHelper.Gems) + rewards[availableReward - 1].reward;
                    PlayerPrefs.SetInt(PlayerPrefsHelper.Gems, newGems);

                    MainMenuManager.Instance.bulletsDisplayText.text = newGems.ToString();  
                }

                if (rewards[availableReward - 1].unit == "Gold")
                {
                    var newGold = PlayerPrefs.GetInt(PlayerPrefsHelper.Gold) + rewards[availableReward - 1].reward;
                    PlayerPrefs.SetInt(PlayerPrefsHelper.Gold, newGold);

                    MainMenuManager.Instance.coinsDisplayText.text = newGold.ToString();
                }

                if (rewards[availableReward-1].unit == "Character")
                {
                    //TODO: GIFT PLAYER A CHARACTER. FARRUKH
                }
            }
